...
include_once("../model/class_material_bd.php");
  ...
  if($whatarray == 'material'){// se for igual produto exclui os materiais do adicionar produto
      ...
      echo '<table style="width:100%" >';
        for($aux = 0; $aux < count($_SESSION['produto'][$whatarray]); $aux++){        
            if($aux%2==0)
               echo '<tr style="background-color:#ccc;">';
            else
              echo '<tr style="background-color:#ddd;">';
                     $id_qtd = explode(":",$_SESSION['produto'][$whatarray][$aux]);
                     
                     $res = Material::get_material_id($id_qtd[0]);
                     echo '<td ><span>'.$res->nome.': </span></td><td><input  id="'.$res->id.':'.$id_qtd[1].'" onchange="increment(this.id)" style="width:30%; background-color: rgba(230,230,230,0.5)" type="number" value="'.$id_qtd[1].'"></td><td><a name="'.$res->id.':'.$id_qtd[1].'" style="cursor:pointer"  onclick="apagar(this.name,\''.$whatarray.'\')"><img style="width:15px" src="../images/delete.png"></a></td>';
